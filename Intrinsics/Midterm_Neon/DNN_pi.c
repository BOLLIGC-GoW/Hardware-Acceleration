...
//This is the re-writing of the Forward function for NEON optimization
             ...
             float *L1_Z, float *L1_Output)
{
    ...
    float sum;
    int loc; 
    float values[5] = { 0,0, 0.0, 0.0, 0.0};
	...
	*/
	...
	float32x4_t *v4_L0_Input = (float32x4_t*) L0_Input;
	float32x4_t *v4_L1_Weights = (float32x4_t*) L1_Weights;


	//Setting a zero vector to be used to hold products
	float32x4_t product_vector = vld1q_f32(values);

	//Loop unrolling
	for(loc = 0; loc < L0_Size/4; loc++){
		//continuously mutiply inputs and weights. Saving in accumulator vector (product vector)
            	product_vector = vaddq_f32(product_vector, vmulq_f32(v4_L0_Input[loc] , v4_L1_Weights[loc + 4*loc]));

        }


	//sum elements together for 'sum'
	vst1q_f32(temp, product_vector);
